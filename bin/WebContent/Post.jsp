...
<div class="row">
	<div class="col-xs-12">
		<h1>${post.title}</h1>
		<span class="postMeta">Post Type: ${post.postType} - Posted in
			${post.category} by ${post.username} on ${post.postDate}. Access
			level: ${post.accessLevel}</span>
		<p>"${post.content}"</p>
		<span class="expand"> <c:if test="${(isAdmin == true)}">
