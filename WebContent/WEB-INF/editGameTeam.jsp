...
<%@ page import="utilities.UserDao"%>
			...
			%>
			<tr>
				<td class="col-md-2 col-xs-12 form-group">
						...
						<div class="col-xs-12">
							<select name="userID" class="form-control" id="inptUserID">
								<c:forEach items="${users}" var="user">
							 		<option value="${user.userid}" ${tm.userid == user.userid ? 'selected' : ''}>${user.firstName} ${user.lastName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
				 </td>
				<td class="col-md-4 col-xs-12 form-group">
					<input type="text" class="form-control" value="${tm.score }">
				</td>
			</tr>
