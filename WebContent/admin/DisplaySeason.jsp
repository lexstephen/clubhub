...
%>
		...
		<form action="/clubhub/GameController" method="post" class="form" role="form">
			S ${season.id}: ${season.year }-${season.gender }-${season.season } <b>${season.startDateFullYear	 }</b> ${season.dayOfWeek } ${season.startTime } (${season.duration } weeks)<br> 
			
			
	<c:forEach items="${games}" var="game">
		<a href="${pageContext.request.contextPath}/admin/Game.jsp?gameID=${game.id }" class="btn btn-default">Game ${game.week}</a> ${game.dayOfWeek}, ${game.scheduledDateFullYear} at ${game.startTime}<br>
		</c:forEach>
