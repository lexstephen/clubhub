...
<div class="form-group">
	...
	<div class="row">
		<div class="<c:if test="${!empty errorNewPostTitle}">has-error</c:if>">
			...
			<div class="col-xs-5">
				<input class="form-control" maxlength="50" type="text" name="blogTitle" value="<c:choose><c:when test="${!empty errorNewPostTitle}">${blogTitle}</c:when><c:otherwise>${post.title}</c:otherwise></c:choose>">
			</div>
		</div>
	...
	<div class="row">
		<div class="form-group">
			<textarea id="blogContent" name="blogContent" class="form-control" rows="16"><c:choose><c:when test="${!empty errorNewPostContent}">${blogContent}</c:when><c:otherwise>${post.content}</c:otherwise></c:choose></textarea><br><br>
		</div>
	</div>
	<div class="row">
		<div class="col-xs-2">
